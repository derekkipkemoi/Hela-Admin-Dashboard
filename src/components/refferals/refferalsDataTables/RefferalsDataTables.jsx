...
class RefferalsDataTables extends Component {
  state = {
    selectedPage: 1,
    pageSize: 5,
    data: dataData,
  ...
  filterData(inputValue) {
    ...
    const filtered = data.filter(
      (item) =>
        ...
        item.refferedUser.includes(inputValue) ||
        item.referalType.includes(inputValue) ||
            ...
            {data.slice(0, this.state.pageSize).map((dataItem, index) => {
              return (
                <tr className="data-table-body-row" key={dataItem.loanRef}>
                  ...
                  <td>{dataItem.refferedUser}</td>
                  <td>{dataItem.referalType}</td>
...
export default RefferalsDataTables;
