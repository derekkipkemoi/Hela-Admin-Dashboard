...
class PromotionsUserReferalsDataTables extends Component {
  state = {
    selectedPage: 1,
    pageSize: 5,
    data: dataData,
  ...
  filterData(inputValue) {
    ...
    const filtered = data.filter(
      (item) =>
        item.referrerName.includes(inputValue) ||
        item.referrerPhone.includes(inputValue) ||
        item.points.toString().includes(inputValue) ||
        item.referredUser.includes(inputValue) ||
        item.referredPhone.includes(inputValue) ||
        item.dateCreated.toString().includes(inputValue) ||
        ...
        <table id="bootstrapDataTable" className="data-table-class table">
          ...
          <tbody>
            {data.slice(0, this.state.pageSize).map((dataItem, index) => {
              return (
                <tr className="data-table-body-row" key={dataItem.referredPhone}>
                  <td>{dataItem.referrerName} </td>
                  <td>{dataItem.referrerPhone}</td>
                  <td>{dataItem.points}</td>
                  <td>{dataItem.referredUser}</td>
                  <td>{dataItem.referredPhone}</td>
                  <td>{dataItem.status}</td>
                  <td>{dataItem.dateCreated.toDateString()}</td>
                </tr>
              );
            })}
          </tbody>
...
export default PromotionsUserReferalsDataTables;
