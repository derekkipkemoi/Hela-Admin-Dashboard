...
function createData(name, phone, amount, paymentFor, mpesaCode, createdAt) {
  return {
    name,
    phone,
    amount,
...
class MpesaReceivedPaymentDataTables extends Component {
  state = {
    selectedPage: 1,
    pageSize: 5,
    data: dataData,
  ...
  filterData(inputValue) {
    ...
    const filtered = data.filter(
      (item) => 
        ...
        item.paymentFor.toString().includes(inputValue) ||
        item.mpesaCode.toString().includes(inputValue) ||
          ...
          <div className="col d-flex justify-content-end col-md-3 col-xl-3">
            ...
            <div className="actions-top-icons-datatables">
              ...
              <ReactTooltip id="printMessage" type="info" effect="solid">
                <span>Print M-PESA Received List</span>
              </ReactTooltip>
            </div>
          </div>
        </div>
            ...
            {data.slice(0, this.state.pageSize).map((dataItem, index) => {
              return (
                <tr className="data-table-body-row" key={dataItem.nationalId}>
                  ...
                  <td>{dataItem.paymentFor}</td>
                  <td>{dataItem.mpesaCode}</td>
...
export default MpesaReceivedPaymentDataTables;
