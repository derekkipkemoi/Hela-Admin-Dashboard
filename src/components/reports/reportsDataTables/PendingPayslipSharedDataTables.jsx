import {
  ...
  import TablesMoreActions from "./TablesMoreActions";
  ...
  let STATUSES = [
    ...
    "Update Payroll",
    "Pending DD - Booked",
    "Pending Payslip Share",
    "@Payroll - DD Pending",
    "@Payroll - Repeat Clients",
    "Declined By Clients",
    "Stop Order",
    "Pending Decline",
    "Compliance",
  ...
  dataData = dataData.filter((item) => item.status === "Pending Payslip Share")
  
  class PendingPayslipSharedDataTables extends Component {
    state = {
      selectedPage: 1,
      pageSize: 10,
      data: dataData,
              ...
              {data.slice(0, this.state.pageSize).map((dataItem, index) => {
                return (
                  <tr className="data-table-body-row" key={dataItem.nationalId}>
                    ...
                    <td>{dataItem.type}</td>
                    <td>{dataItem.borrower}</td>
                    <td>{dataItem.datasheetNumber}</td>
  ...
  export default PendingPayslipSharedDataTables;
