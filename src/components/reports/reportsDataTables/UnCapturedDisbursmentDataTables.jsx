...
const columns = [
  ...
  {
    name: "Trans. Completed Time",
  },
];

let dataData = [];

function createData(amount, transactionId, name, time) {
  return {
    amount,
    transactionId,
    name,
...
class UncapturedDisbursmentDataTables extends Component {
  state = {
    data: dataData,
    selectedPage: 1,
    pageSize: 10,
  ...
  filterData(inputValue) {
    ...
    const filtered = data.filter(
      (item) =>
        ...
        item.time.toString().includes(inputValue)
    );
        ...
        <table id="bootstrapDataTable" className="table-wrapper table">
          ...
          <tbody>
            {this.state.data
              ...
              .map((dataItem, index) => {
                return (
                    <tr className="data-table-body-row" key={dataItem.transactionId}>
                    <td>{dataItem.amount} </td>
                    <td>{dataItem.transactionId}</td>
                    <td>{dataItem.name}</td>
                    <td>{dataItem.time.toDateString()}</td>
                  </tr>
                );
              })}
          </tbody>
...
export default UncapturedDisbursmentDataTables;
